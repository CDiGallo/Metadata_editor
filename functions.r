...
#########################################################################
############# title_generator(no function run only once) ------------------------------

title_generator <- function(endpoint){
...
triple_generator <- function(df, endpoint){


metadata <- df %>% select("Subject","Predicate","Object")
metadata
...
metadata
metadata$s <- ifelse(grepl("^http", metadata$Subject), paste0("<",metadata$Subject,">"),paste0("\"",metadata$Subject,"\"")) #All entries starting with "http" need to have "<>" at start and finish because of lindas (html-datatype). Otherwise it is now a string (better this later)
metadata$URI <- ifelse(grepl("^http", metadata$Predicate), paste0("<",metadata$Predicate,">"),paste0("\"",metadata$Predicate,"\""))
metadata$o <- ifelse(grepl("^http", metadata$Object), paste0("<",metadata$Object,">"),paste0("\"",metadata$Object,"\""))
...
# get_data <- function(endpoint, query,my_dataset){
#   
#   # my_dataset <-    "https://energy.ld.admin.ch/sfoe/bfe_ogd17_fuellungsgrad_speicherseen/5" #my_dataset
#   if(is.na(my_dataset)==TRUE){
#     #print(my_dataset)=="") {
